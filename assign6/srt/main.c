...
int nthreads;
scene_t scene;
double colorArr[width][height][3];

void* splitFunc(void* args) {
  Vec3 camera_pos;
  ...
  /* 
     Process every nthreads pixel, starting at the threadNum pixel
     This ensures that every thread handles a different set of pixels
     and that all pixels are still handled
  */
...
main( int argc, char **argv )
{
    ...
    int thread_nums[nthreads];
    int ret, i;
    
    //create threads and have each one execute splitFunc with i as threadNum
    for (i = 0; i < nthreads; i++) {
      thread_nums[i] = i;
      ret = pthread_create(&tids[i], NULL, splitFunc, (void*) &thread_nums[i]);
      if (ret) {
